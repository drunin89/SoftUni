...
namespace MentorGroup
{
    class MentorGroup
    {
        ...
        private static List<Student> GetStudentsInfo()
        {
            ...
            {
                var usersDates = command.Split(',', ' ').ToList();
                string name = usersDates[0];
                usersDates.RemoveAt(0);
                var dates = usersDates.Select(x => DateTime.ParseExact(x, "dd/MM/yyyy", CultureInfo.InvariantCulture)).ToList();
                ...
                {
                    var student = new Student() { Name = name, Dates = dates, Comments = new List<string>() };
